...
TriviaBot.OnEvent = function(self,event,...)
	...
	elseif (event == "CHAT_MSG_CHANNEL_NOTICE") then
		...
		TriviaBot.UpdateServerChannels(action,channelServerID,channelIndex,channelName)
	...
	elseif (event == "SCAN_CHANNELS") then TriviaBot.UpdateServerChannels();
...
----------------------------------------------------------------------------
-- Server channel discovery / refresh
----------------------------------------------------------------------------
function TriviaBot.UpdateServerChannels(action,channelServerID,channelIndex,channelName)
	local tempAllChannels = {GetChannelList()}
	local tempServerChannels = {EnumerateServerChannels()}
	local numServerChannels = tCount(tempServerChannels)
	local numChannels = tCount(tempAllChannels)
	if numServerChannels == 0 or numChannels == 0 then
		TriviaBot.Schedule("SCAN_CHANNELS",5)
		return
	end
	local loc = SERVER_CHANNEL_NUM_TO_LOC[numServerChannels]
	if not loc then
		TriviaBot.PrintError(format("Unexpected number of server channels:%d",numServerChannels))
		return
	end
	for orderid,globalName in pairs(tempServerChannels) do
		if not TB_Chat_Restricted[strlower(globalName)] then TB_Chat_Restricted[strlower(globalName)] = true end
	end
	if not action then -- full refresh / initial update
		for k,v in pairs(tempAllChannels) do
			if k%2 == 0 then
				local chIdx = tempAllChannels[k-1]
				local id, chName = GetChannelName(chIdx)
				local lv = strlower(v)
				if TB_Chat_Restricted[lv] and (chName) then TB_Chat_Restricted[lv] = strlower(chName) end
				if v == SERVER_CHANNEL_INTERNAL_ID[GENERAL_IID] then
					if not (IsInInstance()) then
						TB_ServerChannels[v] = {["id"]=id,["fullName"]=chName} -- globalName = {id,fullName}
					end
				end
			end
		end
	else
		if channelServerID == GENERAL_IID then
			if (IsInInstance()) or action == "YOU_LEFT" or action == "SUSPENDED" then
				TB_ServerChannels[SERVER_CHANNEL_INTERNAL_ID[GENERAL_IID]] = nil
				TriviaBot.CheckChannel()
			elseif action == "YOU_JOINED" or action == "YOU_CHANGED" then
				TB_ServerChannels[SERVER_CHANNEL_INTERNAL_ID[GENERAL_IID]] = {["id"]=channelIndex,["fullName"]=channelName}
			elseif action == "THROTTLED" then
				TB_ServerChannels[SERVER_CHANNEL_INTERNAL_ID[GENERAL_IID]] = nil
				TriviaBot.CheckChannel()
				TriviaBot.PrintError(L.TB_ERROR_CHANNELTHROTTLED)
				TriviaBot.Print(L.TB_PRINT_THROTTLEHELP)
			end
		end
	end
end
...
function TriviaBot.CheckChannel(event)
	...
	(chat == "officer" and not TriviaBot.IsOfficer()) or
...
function TriviaBot.InitEnd(self)
	TriviaBot.UpdateServerChannels()
	-- Initialize the GUI
